...
local names={'cinder_rival','ash_monk','ash_shogun','gate_guard','twin_cut_retainer','moonlit_master','earth_sentinel','iron_vanguard','mountain_regent','gale_assassin','coast_ronin','tempest_sovereign'}
for _,name in ipairs(names) do
 local s=app.open(app.fs.joinPath(src,name..'.aseprite'))
 assert(s.width==48 and s.height==48 and #s.frames==4 and #s.layers==1)
 assert(#s.tags==1 and s.tags[1].name=='attack' and s.tags[1].fromFrame.frameNumber==1 and s.tags[1].toFrame.frameNumber==4)
...
print('PASS Aseprite reopened 12 editable sources; 48 source/export poses, attack tags, 100ms timing')
